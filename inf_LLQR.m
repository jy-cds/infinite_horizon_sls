...
%     K{i} = dlqr(lqr_A, lqr_B, lqr_Q, lqr_P);
    ...
    psi{i} = -pinv(full(B_bb))*A_bn -(eye(size(B_nn,2))-pinv(full(B_bb))*B_bb)*K{i}; % This is C_k. D_k is C_k times standard basis.
    phi{i} = lqr_A - lqr_B*K{i}; % This is A_k for each position i. B_k is simply multiplying A_k by standard basis.
